...
OpenClaw ↔ Cursor CLI 桥接服务：让如流 / 钉钉 / 飞书等聊天工具里的请求走 Cursor 订阅，同享 IDE 侧规则与上下文，省钱且稳。

- **这是什么**：一层极薄的 HTTP「胶水」，把 OpenClaw 的 OpenAI 接口请求转译为本机 `cursor-agent` 子进程调用，让你在任意支持 OpenClaw 的 IM 里，远程驱动本地 Cursor 工作区。
- **能解决什么问题**：统一 Token 消耗通道（都走 Cursor 订阅）、复用 `.cursor/rules` 与本地文件上下文，避免「手机端聊半天回到 IDE 全部重来」的割裂。
...
以下步骤从零开始，装好即可在 OpenClaw / 聊天工具里用 Cursor 回复。
...
5. **重启 OpenClaw Gateway**，在你绑定的聊天工具里发一条消息测试。
